...
def create_client(
    *,
    ...
    # Allow .env to override defaults without changing call sites
    ...
    def _get_env_bool(name: str, default: bool) -> bool:
        ...
        if v in ("1", "true", "yes", "on"):  # truthy
            return True
        if v in ("0", "false", "no", "off"):  # falsy
            return False
        return default

    # Apply overrides
    use_proxy = _get_env_bool("REDUCTO_USE_PROXY", use_proxy)
    proxy_url = os.getenv("REDUCTO_PROXY_URL", proxy_url or None)
    connect_timeout = _get_env_float("REDUCTO_CONNECT_TIMEOUT", connect_timeout)
    read_timeout = _get_env_float("REDUCTO_READ_TIMEOUT", read_timeout)
    write_timeout = _get_env_float("REDUCTO_WRITE_TIMEOUT", write_timeout)
    pool_timeout = _get_env_float("REDUCTO_POOL_TIMEOUT", connect_timeout)
    max_retries = _get_env_int("REDUCTO_MAX_RETRIES", max_retries)
